...
class Grafo:
    ...
    def vertices_nao_adjacentes(self):
        '''
        Percorre-se a matriz de adjacencia e os pares de vertices que tem o valor 0 não são adjacentes
        :nao_adjacentes: lista com os pares de vértices não adjacentes
        '''
        ...
        for i in range(len(self.M)):
            for j in range(len(self.M[i])):
                if(self.M[i][j] == 0):
                    vertice = self.N[i]+ "-" +self.N[j]
                    nao_adjacentes.append(vertice)

        return nao_adjacentes

    def ha_laco(self):
        '''
        Percorre-se a diagonal principal da matriz de adjacencia e caso algum dos valores seja maior que 0 existe laço
        :return: True caso exista laço
    ...
    def ha_paralelas(self):
        '''
        Percorre-se a matriz de adjacencia e caso algum dos seus valores seja maior que 1 existem arestas paralelas
        :return: True caso existam arestas paralelas
        :return: False caso não existam arestas paralelas
        '''
        matriz = self.M
    ...
    def arestas_sobre_vertice(self, v):
        '''
        Dado um determinado vértice, ele é localizado e em seguida são encontradas as arestas que incidem sobre este vértice
        :param v: Vértice passado como parâmetro, deve-se descobrir as arestas sobre este vertice
        :return: Lista com as arestas que incidem sobre o vértice
        '''
        matriz = []
        ...
        for i in range(len(matriz)):
            if (matriz[indice][i] > 0):
                for j in range(matriz[indice][i]):
                    aresta = (self.N[indice] + "-" + self.N[i])
                    arestas.append(aresta)
            elif(matriz[i][indice] > 0):
                for j in range(matriz[i][indice]):
                    aresta = (self.N[i] + "-" + self.N[indice])
                    arestas.append(aresta)
    ...
    def eh_completo(self):
        '''
        Percorre-se a matriz de adjacencia e se algum valor for igual a 0 o grafo não é completo
        :return:True caso o grafo seja completo
        :return: False caso o grafo seja incompleto
        '''
        matriz = self.M

        for i in range(len(matriz)):
            for j in range(len(matriz[i])):
                if(i != j and matriz[i][j] != '-' and matriz[i][j] == 0):
                    return False
        return True
