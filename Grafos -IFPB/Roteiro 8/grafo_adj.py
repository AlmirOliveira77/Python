...
class Grafo:
    ...
    def dfs_recursion(self, v, vertex, edges):
        ...
        for i in range(len(vertex)):
            if edges[index][i] > 0:
                recursion_dfs += self.dfs_recursion(vertex[i], vertex, edges)
                ...
                if edges[a][i]>0:
                    ...
                    for v in range(len(vertex)):
                        if edge_copying[i][v] > 0:
                            list_dfs += self.dfs_recursion(vertex[v],vertex,edge_copying)
                            for j in range(len(vertex)):
