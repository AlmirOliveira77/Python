...
class Grafo_nao_direcionado:
    ...
    def __init__(self, V=None, M=None):
        ...
        for v in V:
            if not(Grafo_nao_direcionado.verticeValido(v)):
        ...
        for i in range(len(V)):
            for j in range(len(V)):
                ...
                aresta = V[i] + Grafo_nao_direcionado.SEPARADOR_ARESTA + V[j]
    ...
    def arestaValida(self, aresta=''):
        ...
        if aresta.count(Grafo_nao_direcionado.SEPARADOR_ARESTA) != Grafo_nao_direcionado.QTDE_MAX_SEPARADOR:
            return False

        # Índice do elemento separador
        i_traco = aresta.index(Grafo_nao_direcionado.SEPARADOR_ARESTA)

        # O caractere separador não pode ser o primeiro ou o último caractere da aresta
        if i_traco == 0 or aresta[-1] == Grafo_nao_direcionado.SEPARADOR_ARESTA:
            return False
    ...
    def verticeValido(self, vertice: str):
        ...
        '''
        return vertice != '' and vertice.count(Grafo_nao_direcionado.SEPARADOR_ARESTA) == 0
    ...
    def existeVertice(self, vertice: str):
        ...
        '''
        return Grafo_nao_direcionado.verticeValido(vertice) and self.N.count(vertice) > 0
    ...
    def __primeiro_vertice_aresta(self, a: str):
        ...
        '''
        return a[0:a.index(Grafo_nao_direcionado.SEPARADOR_ARESTA)]
    ...
    def __segundo_vertice_aresta(self, a: str):
        ...
        '''
        return a[a.index(Grafo_nao_direcionado.SEPARADOR_ARESTA) + 1:]
    ...
    def existeAresta(self, a: str):
        ...
        '''
        existe = False
        if Grafo_nao_direcionado.arestaValida(self, a):
    ...
    def grafo_nao_direcionado(self):
        ...
        dfs = []
        v = vertices[0]
        l = [v]  # guarda os vertices verificados
        fim = False
        ...
        while fim == False:
            ...
            for i in range(len(vertices)):
                for a in range(len(vertices)):
                    ...
                    if vertices[i] == v and ((aresta[i][a] != '-' and aresta[i][a] != 0) or (
                            aresta[a][i] != '-' and aresta[a][i] != 0)) and (vertices[a] in l) == False and (
                            ('{}-{}'.format(vertices[i], vertices[a]) in dfs) == False and (
                            '{}-{}'.format(vertices[a], vertices[i]) in dfs) == False):
                        l += vertices[a]
                        v = vertices[a]
                        dfs += ['{}-{}'.format(vertices[i], vertices[a])]

                    elif vertices[i] == v and a == len(vertices) - 1:

                        if vertices[i] == v and ((aresta[i][a] != '-' and aresta[i][a] != 0) or (
                                aresta[a][i] != '-' and aresta[a][i] != 0)) and (vertices[a] in l) == True and (
                                ('{}-{}'.format(vertices[i], vertices[a]) in dfs) == False and (
                                '{}-{}'.format(vertices[a], vertices[i]) in dfs) == False) and (
                                ('{}-{}'.format(vertices[i], vertices[a]) in arestas_retorno) == False and (
                                '{}-{}'.format(vertices[a], vertices[i]) in arestas_retorno) == False):
                            arestas_retorno += ['{}-{}'.format(vertices[i], vertices[a])]
                            v = l[l.index(vertices[i]) - 1]
                        else:
                            v = l[l.index(vertices[i]) - 1]

                    elif vertices[i] == v and ((aresta[i][a] != '-' and aresta[i][a] != 0) or (
                            aresta[a][i] != '-' and aresta[a][i] != 0)) and (vertices[a] in l) == True and (
                            ('{}-{}'.format(vertices[i], vertices[a]) in dfs) == False and (
                            '{}-{}'.format(vertices[a], vertices[i]) in dfs) == False) and (
                            ('{}-{}'.format(vertices[i], vertices[a]) in arestas_retorno) == False and (
                    ...
                    elif vertices[i] == v and v == vertices[0] and len(l) == len(vertices):
                        fim = True

        print(dfs)
        print(arestas_retorno)
        print()
        return dfs, arestas_retorno
